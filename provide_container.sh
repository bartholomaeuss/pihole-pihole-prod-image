...
provide_container(){
    ...
    ssh -l "${user}" "${remote}" "docker run -p 53:53/tcp -p 53:53/udp -p 8080:80/tcp -p 8443:443/tcp -v /opt/pihole/etc-pihole:/etc/pihole -v /opt/pihole/etc-dnsmasq.d:/etc/dnsmasq.d --restart=unless-stopped ${image}:${tag}"
    exit 0
}
